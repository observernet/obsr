...
#include "ui_zobsrcontroldialog.h"
...
std::set<std::string> ZObsrControlDialog::setSelectedMints;
std::set<CMintMeta> ZObsrControlDialog::setMints;

bool CZObsrControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZObsrControlDialog::COLUMN_DENOMINATION || column == ZObsrControlDialog::COLUMN_VERSION || column == ZObsrControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZObsrControlDialog::ZObsrControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZObsrControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZObsrControlDialog::~ZObsrControlDialog()
{
    delete ui;
}

void ZObsrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZObsrControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZObsrControlWidgetItem* itemDenom(new CZObsrControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZObsrControlWidgetItem *itemMint = new CZObsrControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zobsrTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zobsrTracker->GetSpendCache(mint.hashStake);
...
void ZObsrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZObsrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZObsr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZObsrControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZObsrControlDialog::GetSelectedMints()
{
...
void ZObsrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
