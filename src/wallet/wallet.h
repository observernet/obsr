...
#include "zobsr/zobsrmodule.h"
#include "zobsr/zobsrwallet.h"
#include "zobsr/zobsrtracker.h"
...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zobsrspends need to use the cs_spendcache
...
// Possible states for zOBSR send
enum ZerocoinSpendStatus {
    ZOBSR_SPEND_OKAY = 0,                            // No error
    ZOBSR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZOBSR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZOBSR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZOBSR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZOBSR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZOBSR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZOBSR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZOBSR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZOBSR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZOBSR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZOBSR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZOBSR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZOBSR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZOBSR_SPENT_USED_ZOBSR = 14,                      // Coin has already been spend
    ZOBSR_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZOBSR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZObsrBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZOBSROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzobsrAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzOBSRTracker> zobsrTracker;
    ...
    void setZWallet(CzOBSRWallet* zwallet)
    {
        zwalletMain = zwallet;
        zobsrTracker = std::unique_ptr<CzOBSRTracker>(new CzOBSRTracker(strWalletFile));
    }

    CzOBSRWallet* getZWallet() { return zwalletMain; }
    ...
    void setZObsrAutoBackups(bool fEnabled)
    {
    ...
    /** zOBSR reset */
    boost::signals2::signal<void()> NotifyzOBSRReset;
