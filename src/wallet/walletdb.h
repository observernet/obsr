...
public:
    ...
    bool WriteZOBSRSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZOBSRSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZOBSRSeed_deprecated(uint256& seed);
    bool EraseZOBSRSeed();
    bool EraseZOBSRSeed_deprecated();

    bool WriteZOBSRCount(const uint32_t& nCount);
    bool ReadZOBSRCount(uint32_t& nCount);
