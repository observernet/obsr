...
private:
    ...
    void ThreadLightZOBSRSimplified() {
        RenameThread("obsr-light-thread");
        ...
        while (true) {
            try {
                ...
                LogPrintf("%s pop work for %s \n\n", "obsr-light-thread", genWit.toString());
                ...
                if (!pIndex) {
                    ...
                    if (blockHeight >= Params().Zerocoin_Block_V2_Start()) {
                        ...
                        try {
                            ...
                            LogPrintStr(std::string("ThreadLightZOBSRSimplified: ") + e.message + "\n");
                        ...
                        if (!res) {
                            ...
                            ss << heightStop;
                            if (genWit.getPfrom()) {
                                LogPrintf("%s pushing message to %s ", "obsr-light-thread", genWit.getPfrom()->addrName);
                                genWit.getPfrom()->PushMessage("pubcoins", ss);
                            } else
                                LogPrintf("%s NOT pushing message to %s ", "obsr-light-thread", genWit.getPfrom()->addrName);
                        }
                    } else {
                        // Rejects only the failed height
                        rejectWork(genWit, blockHeight, NON_DETERMINED);
                ...
                PrintExceptionContinue(&e, "lightzobsrthread");
                break;
            }
        }
    ...
    void rejectWork(CGenWit& wit, int blockHeight, uint32_t errorNumber) {
        if (wit.getStartingHeight() == blockHeight){
            LogPrintf("%s rejecting work %s , error code: %s", "obsr-light-thread", wit.toString(), errorNumber);
            CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
