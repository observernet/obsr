...
CzOBSRWallet::CzOBSRWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zobsr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZOBSRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZOBSRSeed_deprecated()) {
                    LogPrintf("%s: Updated zOBSR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zobsr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zobsr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzOBSRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZOBSRCount(nCountLastUsed);
    else if (!walletdb.ReadZOBSRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzOBSRWallet::Lock()
{
    seedMaster = 0;
}

void CzOBSRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzOBSRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZOBSR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzOBSRWallet::LoadMintPoolFromDB()
{
...
void CzOBSRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzOBSRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzOBSRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zobsrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzOBSRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZOBSR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzOBSRWallet::SeedToZOBSR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzOBSRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzOBSRWallet::GenerateDeterministicZOBSR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzOBSRWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
