...
- [Development guidelines](#development-guidelines)
    - [General OBSR Core](#general-obsr-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
OBSR Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play OBSRs (tOBSR)" on the test network, if you
...
OBSR Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_obsr
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_obsr --log_level=test_suite
$ valgrind -v --leak-check=full src/obsrd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_obsr.coverage/index.html`.
```
...
- BitcoinMiner : Generates OBSRs (if wallet is enabled).
...
However, in open source software such as OBSR Core, where everyone uses
...
pay attention to for reviewers of OBSR Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: OBSR Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against OBSR Core so that they can be integrated
...
file descriptors it will cause problems with OBSR's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `obsr-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `obsr-cli` and the GUI debug console use this table to determine how to
